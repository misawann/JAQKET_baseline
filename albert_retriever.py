...
def render_con2sen(contexts):
    mecab = MeCab.Tagger("-Owakati")
    sentences = []
    sentence_pos = []

    for context_i, context in enumerate(contexts):
        for sentence in re.findall("[^。]+。?", context):
            sentences.append(mecab.parse(sentence).strip())
            sentence_pos.append(context_i)

    return sentences, sentence_pos

def retrieve(args, contexts, question):
    mecab = MeCab.Tagger("-Owakati")
    sentences, sentence_pos = render_con2sen(contexts)
    ...
    sim_list = cosine_similarity(vectorizer.transform([mecab.parse(question)]), vecs)
    top10s = np.array(sim_list[0]).argsort()[::-1][:10]
    contexts_top10 = ['']*len(contexts)
    for top10 in top10s:
        context_i = sentence_pos[top10]
        context = contexts_top10[context_i]
        context += sentences[top10]
        contexts_top10[context_i] = context

    return contexts_top10
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        choices_features = []
        contexts = retrieve(args, example.contexts, example.question)
        for ending_idx, (context, ending) in enumerate(
            zip(contexts, example.endings)
        ):
...
def main():
    ...
    parser.add_argument(
        ...
        help="")
    # Other parameters
    parser.add_argument("--config_name", default="./model/config.json", type=str, help="")
    parser.add_argument("--tokenizer_name", default="./model/spiece.model", type=str, help="")
    ...
    if args.do_train:
        ...
        if (os.path.isfile(
            ...
            # Load in optimizer and scheduler states
            model = AlbertForMultipleChoice.from_pretrained(t_output_dir)  # , force_download=True)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = AlbertForMultipleChoice.from_pretrained(args.output_dir)
        tokenizer = AlbertTokenizer.from_pretrained(args.output_dir)
        model.to(args.device)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = AlbertForMultipleChoice.from_pretrained(checkpoint)
            # initialize_params(model) #########
            model.to(args.device)
