...
def retrieve(args, contexts, question):
    ...
    for context in contexts:
        cols = context.strip().split('\t')
        contexts_wakati.append(mecab.parse(cols[0]).strip())
    stop_words = []
    if args.stop_words:
    ...
    vecs = vectorizer.fit_transform(contexts_wakati)
    sim_list = cosine_similarity(vectorizer.transform([mecab.parse(question)]), vecs), reverse=True)
    top5s = np.array(sim_list).argsort()[::-1][:5]
    contexts_top5 = []
    for i, context in enumerate(contexts):
        if i in top5s:
            contexts_top5.append(context)
        else:
            contexts_top5.append('')
    return contexts_top5
